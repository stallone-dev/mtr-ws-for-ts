...
import type { ConsultarMtrResponseDTO, GeradorResponseDTO } from "./types/dto.type.ts";
...
export class GeradorClient extends BaseMtrWsClient {
    ...
    public async gerarMtr(payload: unknown): Promise<GeradorResponseDTO> {
        const span = tracer.startSpan("GeradorClient.gerarMtr");
        logger.info("Gerador: Gerando MTR");
        try {
            const result = await gerarMtr({ token: this.token, baseUrl: this.baseUrl }, payload);
            span.setAttribute("resultado.manNumero", result.manNumero);
            return result;
        } catch (error) {
            logger.error("Erro em gerarMtr:", { error });
            span.recordException(error as Error);
            throw error;
        } finally {
            span.end();
